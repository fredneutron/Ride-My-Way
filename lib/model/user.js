...
export default class user {
  ...
  static signUp(re, res) {
    ...
    dbConnect.connect();
    dbConnect.query('SELECT * FROM users WHERE user_email = $1', [re.email], (error, result) => {
      if (result) {
  ...
  static logIn(req, res) {
    dbConnect.connect();
    dbConnect.query('SELECT id, user_password,user_email FROM users WHERE LOWER(user_email) = LOWER($1)', [req.body.email], (err, result) => {
      if (result) {
        if (result.rowCount !== 0 &&
