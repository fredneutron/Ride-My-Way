...
class RideModel {
  static getAllRide(req, res, today) {
    dbConnect.connect((error, client, done) => {
      return error
        ...
        : client.query('SELECT * FROM rides WHERE ride_date <= $1', [today], (err, result) => {
          done();
          if (result) {
  ...
  static getActiveUser(req, res, today) {
    dbConnect.connect((error, client, done) => {
      return error
        ? res.status(500).json(error)
        : client.query('SELECT * FROM rides WHERE ride_owner_id = $1 AND ride_date <= $1', [req.params.id, today], (err, result) => {
          done();
          if (result) {
            return res.status(200).json({
              total: result.rowCount,
  ...
  static getAllRidesUser(req, res) {
    dbConnect.connect((error, client, done) => {
      return error
        ? res.status(500).json(error)
        : client.query('SELECT * FROM rides WHERE ride_owner_id = $1', [req.params.id], (err, result) => {
          done();
          if (result) {
            return res.status(200).json({
              total: result.rowCount,
  ...
  static getRide(req, res) {
    dbConnect.connect((error, client, done) => {
      return error
        ? res.status(500).json(error)
        : client.query('SELECT * FROM rides WHERE ride_id = $1', [req.params.id], (err, result) => {
          done();
  ...
  static searchRide(req, res, today) {
    dbConnect.connect((error, client, done) => {
      return error
        ? res.status(500).json(error)
        : client.query('SELECT * FROM rides WHERE ride_destination = $1 AND ride_location = $2 AND ride_date <= $3', [
  ...
  static addNewRide(req, res) {
    dbConnect.connect((error, client, done) => {
      return error
        ? res.status(500).json(error)
        : client.query('SELECT picture FROM users WHERE id = $1', [req.body.creator_id], (err, result) => {
          done();
          if (result) {
  ...
  static addReq(req, res) {
    dbConnect.connect((error, client, done) => {
      return error
        ? res.status(500).json(error)
        : client.query('INSERT INTO ride_request(ride_id,passenger_id,req_status) VALUES($1,$2,$3)', [
          req.params.rideId,
  ...
  static getRideRequests(req, res) {
    dbConnect.connect((error, client, done) => {
      return error
        ? res.status(500).json(error)
        : client.query('SELECT * FROM ride_request WHERE ride_id = $1', [req.params.rideId], (err, result) => {
          done();
          if (result) {
            return res.status(200).json({
              total: result.rowCount,
  ...
  static getRequest(req, res) {
    dbConnect.connect((error, client, done) => {
      return error
        ? res.status(500).json(error)
        : client.query('SELECT * FROM ride_request WHERE passenger_id = $1', [req.params.id], (err, result) => {
          done();
          if (result) {
            return res.status(200).json({
              total: result.rowCount,
  ...
  static getAcRideRequest(req, res) {
    dbConnect.connect((error, client, done) => {
      return error
        ? res.status(500).json(error)
        : client.query('SELECT * FROM ride_request WHERE passenger_id = $1 AND req_status = $2', [req.params.id, 'Accept'], (err, result) => {
          done();
          if (result) {
            return res.status(200).json({
              total: result.rowCount,
  ...
  static rideRequestOption(req, res) {
    dbConnect.connect((error, client, done) => {
      return error
        ? res.status(500).json(error)
        : client.query('UPDATE ride_request SET req_status = $1 WHERE ride_id = $2 AND req_id = $3', [
