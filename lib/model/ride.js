...
export default class ride {
  ...
  static addNewRide(req, res) {
    dbConnect.connect();
    dbConnect.query('SELECT user_vehicle_picture FROM users WHERE id = $1', [req.body.creator_id], (err, result) => {
      if (result) {
        dbConnect.query('INSERT INTO rides(ride_location,ride_destination,ride_date,ride_time,ride_prize,ride_owner_id,ride_seat,ride_mtp,ride_image) VALUES($1,$2,$3,$4,$5,$6,$7)', [
          ...
          req.body.seat,
          req.body.mtp,
          result.rows[0].user_vehicle_picture], (er, re) => {
          if (re) return res.status(200).json(result.rowCount);
          if (er) return res.status(500).json(err.stack);
        });
      }
      if (err) return res.status(500).json(err.stack);
    });
